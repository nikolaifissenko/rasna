...
- **Gas:** diesel is currently ~€2.00/L (elevated — accise discount expired, geopolitical spike, July 2026). At ~10 L/100km for a 9-seat diesel van and an estimated ~300–500 km over the trip week (airport transfer + day trips to Lago di Vico, Villa Lante, Tarquinia, Viterbo), that's roughly **€70–120 in fuel**.
- **Collision/CDW insurance:** not a separate line item — covered automatically by Amex Platinum's built-in rental car CDW/casco coverage, so no need to pay the rental company's insurance excess/waiver on top of the base rate.
- That leaves van + gas at roughly €570–920/group (≈€70–115/guest at 8 guests), within the €140/guest budgeted here, with the remainder covering the local driver/guide's time.
**Note on lodging:** this is now a real, confirmed rate (La Ripa, in Blera, run by Antonella — €56/night per Booking.com, single-adult/single-room search) rather than an estimate, replacing the earlier €170/~€57 placeholder. Two caveats to resolve before locking this in: (1) the €56 rate is for 1 adult/1 room/1 night — confirm actual per-room or group rates directly with Antonella, since group bookings may price differently than a single-room search; (2) La Ripa can only host **~2 couples (4 guests) and doesn't take single travelers** — rooms are double-occupancy only. For the assumed 8-guest group size, this covers roughly half the group at best (and only the paired-up half), so at least one more lodging partner is needed both to house the rest of the group and to cover any solo travelers.

**Beccone Albergo is confirmed as the chosen second lodging partner** — it's in Blera, has ~20 rooms (up to 60 guests) including singles, triples, and family rooms, covering exactly what La Ripa can't (single travelers and the rest of the group). Indicative rates found on a third-party listing site (not Booking.com, which doesn't list Beccone for Blera): singles €35–40, doubles €54–60, triples €68–78, family €75–88/night (breakfast included) — **rates still unverified, needs direct confirmation with the property before locking into the cost model.** See `CONTATTI_LOCALI.md` for details and contact info.

**Two more lodging options added, pending confirmation:**
- **Casamatta a Blera — the most promising option found so far.** A centrally-located house in Blera split into 2 connected apartments with **9-guest total capacity** — potentially enough to house the *entire* standard 8-guest group in one place, replacing the need to split across La Ripa + Beccone. Indicative rate ~€86/night (unverified, source unclear whether that's per apartment or the whole property). **Illustrative scenario only, not yet in the cost model:** if €86/night applies per apartment and both are needed to reach 9-guest capacity, that's ~€172/night for the group → ~€516 for 3 nights → **~€57–65/guest for lodging**, well below the current €168/guest La Ripa-based estimate, and it would simplify logistics (one pickup/drop-off point instead of two). This needs a direct call to confirm actual pricing and layout before it can replace the La Ripa + Beccone combo in the numbers above.
- **La Dimora degli Etruschi** — a rural relais/agriturismo with a pool in Civitella Cesi (same hamlet as Davide Paolocci and the horse stable, ~7km from Blera), listed in one source as an "ostello," hinting at group-friendly capacity. No price found yet — added as an option to explore, not factored into costs until a rate is confirmed.

See `CONTATTI_LOCALI.md` for contact info and links for both.
...
- **Weekly rental, local pickup (Viterbo area):** listings start around **€110/day**, general Italian rates for a 9-seat van run ~€100–150/day; weekly rentals typically get a multi-day discount off that per-day rate. Realistic estimate: **~€500–800/week** (before fuel, extra-km charges, insurance excess, or deposit).
- **Weekly rental, picked up at Fiumicino airport (FCO) instead:** average ~$113/day (~€105/day), cheapest listings from ~$76/day (~€70/day), averaging out to roughly **~€500–750/week** — in the same ballpark as the local Viterbo option, not meaningfully more expensive. The real tradeoff isn't price, it's logistics: Fiumicino has far more selection/companies (Europcar, Sixt, Hertz, Avis, etc.), and it's a natural pickup point if guests are flying into Rome anyway — but check for one-way fees if the van needs to be dropped off somewhere other than Fiumicino at the end of the trip, and confirm whether airport surcharges are already included in the quoted rate.
...
**Insurance:** no need to budget separately for the rental company's collision damage waiver (CDW/casco) — Amex Platinum includes automatic rental-car CDW coverage, which covers this as long as the rental is paid with that card. Still confirm coverage terms/excess directly with Amex before relying on it for a commercial/business rental use case, since consumer card benefits sometimes carve out business use.

**Fuel:** see Section 1 for the gas cost estimate (~€70–120/trip week at current diesel prices).
