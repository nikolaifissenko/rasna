...
- **Ownership: Nikolai runs Rasna independently. No co-founder, no partner, no profit split with anyone, full stop.** Corrected 2026-09-06 after this got reintroduced repeatedly across sessions (see the correction note in `BUSINESS_PLAN.md` and the rewrite of `RASNA_PITCH_IT.md`); do not resurrect "two-founder" or "partnership" framing for Maria Grazia or anyone else from an older cached doc or branch. **Local people (Maria Grazia, Davide, Emiliano, Renzo, Nicolò, etc.) are paid, narrow-scope collaborators, and by default should NOT be named or featured in outward-facing copy** (LinkedIn posts, founder bios, Instagram captions, pitch decks) — describe activities generically (olive harvest, cheese tasting, leatherworking) rather than attaching a host's name, unless Nikolai explicitly names someone for that specific piece of copy. Their real involvement is often much smaller than earlier docs implied (e.g. Maria Grazia's role may be as narrow as "owns a bar in town, maybe hosts an olive activity") — don't inflate it, ask Nikolai directly if a specific person's actual role matters for something being written.
...
## Session, 2026-08-16 — branch-drift incident #8, liability waiver added

**The branch-drift trap happened an eighth time**, same root cause as every prior incident: this session's designated branch (`claude/website-liability-section-pwh5gu`) descended from the dead `claude/magical-franklin-58SKM` lineage, whose bundled `CLAUDE.md` was short, didn't mention `main` at all, and flatly claimed the *static site* deployed from `claude/magical-franklin-58SKM` — the exact wrong claim incidents #2–#7 already warned about. A full liability-waiver page, a printable guest signature form, and several rounds of legal-content refinement were built and merged against that dead branch before the drift was caught. Caught this time by checking real GitHub Pages deploy evidence directly (`mcp__github__actions_list`, `pages-build-deployment` workflow, `head_branch` on every recent successful run) rather than trusting either branch's docs, then confirmed by diffing `origin/main` vs `origin/claude/magical-franklin-58SKM` (201 commits apart one direction, 61 the other — completely different site generations, main has ~30 more activities and a whole family of dedicated pages the other branch never got). The dead branch's copies of `liability-waiver.html`/`cancellation-policy.html` (old Cormorant Garamond/gold-original design, old flat activity list) were left inert there, not ported — same precedent as every prior incident's discarded dead-branch work.

**What actually shipped, rebuilt against `main` with the real, current activity catalog and design system:**
- `liability-waiver.html` — new page, `style.css`-based (matches current Bodoni Moda/bordeaux design, not the old inline-styled version built against the dead branch). Grounded in real law, not boilerplate: Civil Code Art. 1229 (can't exclude liability for gross negligence/wilful misconduct), Art. 1227 (contributory fault if a guest ignores safety instructions), and the EU Package Travel Directive 2015/2302 / Codice del Turismo (Rasna sells accommodation+meals+activities as one package, so liability for death/personal injury caused by Rasna's or a local host's fault can't be capped or excluded by contract — but ordinary inherent risk, absent fault, isn't Rasna's liability). Activity list covers the real current catalog (tombs/necropolis, horseback + donkey riding, hiking, harvest work, asparagus + **mushroom** foraging with an explicit "don't eat anything not confirmed safe by a host" line, wine/cantina tastings, football, festivals), not just the handful the dead branch's stale catalog had.
- `guest-waiver-form.html` + rendered `guest-waiver-form.pdf` (repo root, matching the `RASNA_Pitch_Deck.pdf` precedent — **not** `assets/`, which doesn't exist on `main`) — a separate printable/signable form for guests to fill out before the trip: emergency contact, travel insurance, medical disclosure, a full per-activity checklist, an emergency-medical-treatment authorization, and an optional photo-release checkbox. Not wired into any automated flow yet (no post-payment email) — print-and-sign-on-arrival or manual email for now. Regenerate the PDF (headless Chromium, `--print-to-pdf`, see command in git history) if the HTML changes.
- Linked from three real places guests actually see: the `#policies` section on `index.html` (replaced the thin one-line "Health & wellbeing" note with a real liability/fault-based-liability paragraph + link), a new FAQ entry on `italian-olive-experience-faq.html` ("Is this trip safe? Who's liable if I get hurt?"), and right next to the existing "Cancellation policy" link on `italian-olive-experience-pricing.html`, directly above the real Stripe booking form — the highest-value placement since that's where money changes hands.
- Compliance items flagged to Nikolai but **not resolved** (need him, not code): whether Rasna needs professional liability insurance (RC organizzatore di viaggi) and/or registration as a licensed tour operator under Lazio regional law, since it's legally a package-travel organizer. A webpage/form protects evidentially; it doesn't substitute for either.

If you're a fresh session reading only a `claude/*` branch's bundled `CLAUDE.md` and it doesn't match this file's content almost word-for-word, or gives any deploy branch other than `main` for the static site: stop, run `git log origin/main`, and diff before writing a single line — this is the ninth time this file has had to say some version of that sentence.

---

## Session, 2026-08-20 — branch-drift incident #9, root cause finally identified; price raised, contingency plan added

**The branch-drift trap happened a ninth time**, same shape as #2–#8: this session's designated branch (`claude/remove-dashes-site-9h503c`, descended from the dead `claude/magical-franklin-58SKM` lineage) had a bundled `CLAUDE.md` flatly claiming GitHub Pages deploys the static site from that branch. A full em-dash cleanup pass and the first attempt at a price change (€1,400/€1,800 → €1,825/€2,125) were built and merged against the dead branch before the drift was caught — this time via `git ls-remote --symref origin HEAD` after a live `curl` of rasnaexperience.com showed stale prices post-push. **New this time: the actual root cause was found, not just the symptom.** `git ls-remote --symref origin HEAD` returns `claude/magical-franklin-58SKM` — **the GitHub repository's own default branch is misconfigured**, pointing at the dead lineage instead of `main`. That's *why* every fresh session inherits the wrong bundled `CLAUDE.md` in the first place — it's not random bad luck picking a stale `claude/*` branch, it's the repo's default branch itself. **Still unfixed as of this entry** — it's a one-click change (repo Settings → Branches → change default branch to `main`) that only Nikolai or someone with repo admin access can make; ask him about it directly rather than re-diagnosing this from scratch next time. The dead branch's own `CLAUDE.md` was also corrected in place (2026-08-20 commit) to state the real topology loudly, as a stopgap until the default branch is fixed.

**What actually shipped, correctly, against `main`:**
- Price raised to **€1,825 early bird / €2,125 full** — `worker/` side (`claude/magical-franklin-58SKM`, correct branch for that) and the static side (`main`: `index.html` pricing hub, `italian-olive-experience-pricing.html`/`-itinerary.html`/`-faq.html` including JSON-LD/meta text that doesn't pull from the live API) both updated and confirmed live via `curl`, specifically checked to match each other since a displayed price that doesn't match the Stripe charge is the real risk, not just a stale number.
- **New file `CONTINGENCY_PLAN.md`** (repo root) — per-day weather/host-unavailability backup plan for the November pilot's activities, cross-referencing `ITINERARY_NOV2026.md` and `CONTATTI_LOCALI.md`. Lodging backup confirmed by Nikolai: **Da Beccone** if Casamatta falls through. Several items still flagged as needing him (backup tomb-hunt guide, covered-BBQ-option check with Nicolò, the still-unconfirmed Monday/Saturday dinner venues) — see that file directly, not repeated here.
- `BOOKING_STATUS.md` updated with the same price change and a shorter version of this incident.

If you're a fresh session and this section doesn't match what you were told about the deploy branch: the default branch is still probably misconfigured — check `git ls-remote --symref origin HEAD` yourself and tell Nikolai if it still doesn't say `main`.

---

## Session, 2026-09-01 — branch-drift incident #10 (caught early, no wasted work); two new Day Trips/Festivals cards added

**Branch-drift incident #10, but this time no work was lost.** This session's designated branch (`claude/cerca-eteuscacomics-mlbn1k`) was, once again, descended from the dead `claude/magical-franklin-58SKM` lineage (confirmed via `git branch -a --contains HEAD`). `git ls-remote --symref origin HEAD` still returns `claude/magical-franklin-58SKM` — **the repo's default branch is still not fixed**, this is now the fourth session in a row to confirm that (see the 2026-08-20 entry above for the original diagnosis; still needs Nikolai or another repo admin to go to Settings → Branches and change it to `main` — a one-click fix nobody's made yet). Unlike most prior incidents, this session never wrote to the wrong branch: any time a task touched the static site, the first move was `git checkout -B <task-branch> origin/main` before editing anything, so there was nothing to discover-and-discard this time. Worth normalizing as the default habit for every session, not just a recovery step after getting burned.

**What shipped, both directly on `main` (branch names `site-etruscomics` and its follow-up, both tracking `origin/main`, pushed straight to `main` per the established direct-commit pattern — no PR flow used for the static site in this repo's history):**

1. **EtrusComics added to Festivals & Sagre.** Nikolai attended EtrusComics 2026 (Aug 29-30, Piazza Santa Maria, Blera — a real annual cosplay/comics/manga/videogame festival run by the Comune, Pro Loco, and Università Agraria di Blera, free entry) and sent two of his own phone photos (large dragon/Toothless cosplay props in the piazza). Added a new card to `#cat-festivals` (`images/card-etruscomics.jpg`, one of his two photos, cropped/warm-graded to match the other festival cards, positioned chronologically after the Sant'Ermete card since Aug 29-30 falls right after "last Sunday of August"). Also drafted an English Instagram caption in the Why→How→What voice from `INSTAGRAM_STRATEGY.md` — not logged to that file's content queue since it wasn't confirmed shipped by end of session; if Nikolai posted it, check with him and log it there next time per that file's own tracking convention.

2. **Bomarzo (Sacro Bosco / Parco dei Mostri) added as a new Day Trip.** Nikolai asked for a new location added to the "experiences package," researched (history: commissioned by Pier Francesco "Vicino" Orsini 1547–1580, Mannerist break from Renaissance garden convention, abandoned after his 1585 death, restored in the 1970s by the Bettini family; ~31 km / 35–40 min from Blera; real current pricing €13 adult / €8 reduced (4–13) / free under 4, from sacrobosco.eu, open year-round except Christmas Day), and added to:
   - `BUSINESS_PLAN.md` (Day Trips list) and `FINANCIAL_PLAN.md` (mentioned alongside the other day-trip destinations in §0/§1, and the real €13 ticket price noted inline on the "Activity fees" row of the per-guest cost table) — no new cost-table line item added, since day trips have always been folded into the existing "Local guiding/transport" + "Activity fees" lump sums, not priced individually.
   - `index.html` `#cat-daytrips`: new card (icon: new `#icon-mask` SVG symbol added to the shared defs, since nothing existing fit a "stone monster" theme), placed after Villa Lante & Palazzo Farnese.
